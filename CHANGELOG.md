...
### Changed
- Converted all 6 `dplyr::group_by()`/`dplyr::summarise()` grouped-aggregation call sites in `zi_aggregate.R` and the paired `group_by()`+`slice()`+`ungroup()` combo in `zi_prep_hud.R` to a new internal `group_summarise_base()` helper (`R/zi_utils.R`), as part of Epic L's dplyr removal; the helper matches `dplyr::group_by()`+`summarise()`'s output contract exactly (row/column ordering, `.groups` semantics, NA-as-a-real-level grouping, and NA-vs-NaN distinction in numeric grouping columns), verified via a 4-round adversarial code-review pass and 6 new regression tests; behavior is unchanged, no `rlang` introduced (#132)
- Converted all 7 `dplyr::left_join()` call sites in `zi_aggregate.R`, `zi_crosswalk.R`, `zi_label.R`, and `zi_prep_hud.R` to a new internal `left_join_base()` helper (`R/zi_utils.R`) wrapping Base R `merge()`, as part of Epic L's dplyr removal; the helper restores `merge()`'s row and column ordering to match `dplyr::left_join()`'s contract exactly (`x`'s original row order, `x` columns followed by new `y` columns) and disambiguates colliding non-key column names with `.x`/`.y` suffixes the same way `dplyr::left_join()` does, verified against NA-key matching, one-to-many duplicate-key expansion, and column-name collisions; behavior is unchanged, no `rlang` introduced (#131)
...
- Converted `dplyr::filter()`, `mutate()`, `arrange()`, and `bind_rows()` call sites in `data-raw/build_uds_crosswalk.R` and 18 `dplyr::filter()`/`select()` call sites in `tests/testthat/test_zi_aggregate.R` (discovered as an unstated test dependency during `R CMD check`) to Base R equivalents, and removed `dplyr (>= 1.0.0)` from `DESCRIPTION` Imports, completing Epic L's dplyr removal; `devtools::check()` now runs with 0 errors and 0 warnings (#134)
- Extracted `bind_rows_base()` from `data-raw/build_uds_crosswalk.R` into a new sourceable `data-raw/helpers.R`, and added `tests/testthat/test_data_raw_helpers.R` with offline, fixture-based test coverage (15 assertions) for the helper's union-of-columns, NA-fill, and row-order behavior; no runtime behavior change (#143)
